...
| T3 paid-call reservation and audit | Final-batch T3 security ledger | Implemented locally in API `35a25c56`: paid embedding spend and provider connection tests reserve atomically before actuation, failure/cancellation receipts remain budget-visible, and finalization outages alert without reopening the cap. Ordinary generation-call reservation remains open. |
...
| T3 embedding configuration integrity | Final-batch T3 security ledger | Implemented locally in API `d7477474`: paid-provider transitions validate the request's new key before mutation, cross-provider key reuse is refused, invalid changes return typed 422 responses, and completed-job coverage uses the written status value. Vector upsert/prune/HNSW integrity remains open. |
...
The held T3 branches now contain API commits `a4be506b`, `5a67885f`, `35a25c56`, `a48e534b`, `92c7e2b8`, `d7477474`, and `367145b2` on `fix/t3-security-api` at `367145b2`, plus web commit `a272c8dc` on `fix/t3-security-web`. In addition to the original 55-test API receipt, the new focused slices pass 26 spend tests, 16 KDF tests, 25 limiter tests, 55 embedding tests, and 20 suggestion-safety tests; Ruff, strict changed-source mypy, and diff checks pass. The paid-spend real-PostgreSQL concurrency test collects but cannot execute because PostgreSQL is not listening locally. The web full suite passes 208 files/3,295 tests; type-check, changed-file ESLint, and diff checks pass. Diff-scoped adversarial review followed by reuse, quality, and efficiency simplification materially tightened the implementation. The API's broad synchronous `TestClient` route harness stalls before output in this environment, so it is not claimed as passing. Both branches are local only—not pushed or merged—and no external or runtime state changed.
