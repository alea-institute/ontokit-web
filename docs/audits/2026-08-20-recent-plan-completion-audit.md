...
| U14 upstream delivery map | **Partially complete locally** | `docs/roundup-2026-08/UPSTREAM-DELIVERY-MAP.md` accounts for all 277 cutoff commits and holds feature-seam drafts. API T1 synthesis `d2c31aea` passes 1,580 tests/Ruff/mypy; web auth synthesis `f44592a8` passes 2,772 tests/type/lint/build while preserving optional no-Zitadel mode. The remaining web anonymous/public seam and final pair replay remain; nothing has been sent. |
