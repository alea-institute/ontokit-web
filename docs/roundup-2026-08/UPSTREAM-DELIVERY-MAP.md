...
| API | `fix/t3-security-api` at `367145b2` | 7 T3 security/quality commits after `0a54857a` | Fold provider-network pinning, project-job serialization, paid embedding/connection-test reservations, provider-key HKDF migration, limiter TTL repair, embedding transition/status integrity, and redacted fail-soft alerts into T3; local-only, focused-tested, not pushed or merged |
...
| T3 | LLM configuration and controlled generation | Remaining W1/W4 and A1/A4 plus API `a4be506b`..`367145b2` and web `a272c8dc` | web + API | T1 | large, split into config, generation, and hardening PRs if reviewer requests | Seven held API hardening commits and the web conflict UX are green locally; ordinary-generation reservation plus vector upsert/prune/HNSW integrity remain before synthesis |
