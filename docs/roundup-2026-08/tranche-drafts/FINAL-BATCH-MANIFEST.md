...
| T3 | LLM configuration and controlled generation | one web owner; one API umbrella plus one child owner for each independently mergeable API PR | web configuration/UI PR; API configuration/generation/hardening PRs sized for review | SSRF/DNS-pinning, safe-error, project-job serialization, spend reservation, and vector-integrity residuals are reviewed and green locally | API synthesis is green at `1b8bde28`; finish web synthesis and paired current-upstream replay |
