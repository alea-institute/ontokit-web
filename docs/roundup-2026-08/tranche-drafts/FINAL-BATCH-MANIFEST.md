...
- atomic budget reservation, missing-usage estimation, `test_connection` metering, and failure audit — **partially fixed locally:** API `35a25c56` atomically reserves paid embedding spend and meters connection tests, retains failure/cancellation receipts, and preserves conservative spend when finalization fails. Provider adapters already estimate missing usage. Atomic pre-actuation reservation for ordinary generation calls remains open and blocks real project-key deployment;
- embedding/dedup integrity, project-level job throttling, and merge-triggered reindexing — **partially fixed locally:** API `5a67885f` adds a per-project Redis claim, deterministic queue identity, atomic renewal/reclaim/release, retry-safe cancellation behavior, and ambiguous-enqueue reconciliation; API `d7477474` fixes completed-job coverage lookup and validates paid-provider transitions with the request's new key before mutation. Merge-triggered full refresh already exists and is covered. Concurrent vector upsert/prune consistency and the dimension/HNSW design remain open and block scale claims;
...
Held source heads are API `fix/t3-security-api` at `367145b2` and web `fix/t3-security-web` at `a272c8dc`. After the earlier 55-test receipt, focused slices pass 26 spend tests, 16 KDF tests, 25 limiter tests, 55 embedding tests, and 20 suggestion-safety tests; Ruff, strict changed-source mypy, and diff checks pass throughout. The paid-spend real-PostgreSQL concurrency test collects but cannot run because PostgreSQL is not listening at `127.0.0.1:5433`. The web receipt remains the full 208-file/3,295-test suite, type-check, changed-file ESLint, and diff checks. The API's broader synchronous `TestClient` route harness stalls before producing results in this environment, so direct async route/submission/worker coverage is the current receipt; real-PostgreSQL/Redis replay remains part of final synthesis validation. Neither branch is pushed or merged.
...
D5 source is local API `f3c4251d`. Its PR closes only the D5 control issue and relates the main PR Party issue. It re-encrypts the same stored PAT under the current application key; it does not replace or revoke the GitHub PAT. Deployment and execution remain a separately controlled application-key-rotation step.
