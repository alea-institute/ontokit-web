...
| T4 | Trust ladder and auto-accept | one web trust/UI owner and one API trust/lifecycle owner | linked web/API PR pair | Paired reviewed current-upstream candidates are green at API `8d6fc226` and web `d4ed4581` | Authenticated quiet-period, halt/resume, and TRUSTED-gate UAT receipt |
...
The local API delivery candidate is `upstream-queue/t4-api-synthesis` at `8d6fc226`, based on T3 API `1b8bde28`. Its full suite passes 2,095 tests with two expected integration skips; Ruff is clean; targeted mypy is clean across all 23 changed source files; Alembic has one head; and a disposable PostgreSQL/pgvector database completed empty-to-head upgrade, T4-only downgrade, and re-upgrade. The remaining full-project mypy findings are the pre-existing embedding-model shim, outside T4.

The local web delivery candidate is `upstream-queue/t4-web-synthesis` at `d4ed4581`, based on T3 web `2c6a7813`. All 184 test files and 2,983 tests pass; type-check passes; lint has zero errors and 18 pre-existing warnings; and the optional/no-Zitadel production build emits 22 static pages. Both candidates completed full local `ce-code-review` rosters, validated and applied every actionable finding, and used a local adversarial fallback after external code-egress approval was denied. Both branches are local, unpushed, and unmerged; authenticated DEV UAT remains a send condition rather than a claimed receipt.
