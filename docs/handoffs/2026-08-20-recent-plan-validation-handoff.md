...
1. Synthesize the completed T3 heads `738f20a6` and `a272c8dc` onto refreshed CatholicOS `dev` in local scratch branches; do not push the source branches.
...
The prior documentation state was committed and pushed through `38a58f0b` on `feat/roundup-brainstorm`; this receipt supersedes its earlier T3 head and test counts. The web residual branch `fix/plan-audit-web-residuals` at `4a4dc5bf`, API residual branch `fix/recent-plan-api-residuals` at `0a54857a`, API demo-refresh branch `feat/demo-refresh-scaffold` at `e894f20f`, API demo-isolation branch `feat/demo-project-isolation` at `b5b13d8f`, web demo-UI branch `feat/demo-project-ui` at `d7e490ec`, API D5 branch `fix/pr-party-credential-rewrap` at `f3c4251d`, API T3 branch `fix/t3-security-api` at `738f20a6`, web T3 branch `fix/t3-security-web` at `a272c8dc`, API T1 synthesis branch `upstream-queue/t1-api-synthesis` at `d2c31aea`, and web T1 synthesis branch `upstream-queue/t1-web-synthesis` at `4986135c` are durable local branches only; none is pushed or merged.
