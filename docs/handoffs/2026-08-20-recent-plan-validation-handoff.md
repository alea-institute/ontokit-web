...
The autonomous T3 security branch now ends at API `367145b2`. In addition to `a4be506b` (exact-origin/DNS-pinned provider networking) and `5a67885f` (project-level quality-job serialization), it contains `35a25c56` (atomic paid-embedding and connection-test reservations), `a48e534b` (domain-separated provider-key HKDF with legacy migration), `92c7e2b8` (crash-repairable trust/PR Party counter TTLs), `d7477474` (safe paid-provider transitions and completed-job status truth), and `367145b2` (structured redacted fail-soft alerts). Web branch `fix/t3-security-web` remains at `a272c8dc`, preserving structured API bodies while presenting safe project-analysis conflict messages. The new API slices pass 26 spend, 16 KDF, 25 limiter, 55 embedding, and 20 suggestion-safety tests, with Ruff, strict changed-source mypy, and diff checks green; the paid-spend PostgreSQL concurrency test collects but cannot run because local PostgreSQL is not listening. Prompt/output IRI controls, Turtle parsing, server-derived mint authorization, branch-serialized submit idempotency, merge-triggered refresh, and PR Party retry semantics were verified in the held source. Atomic reservation for ordinary generation calls and deeper vector upsert/prune/HNSW integrity remain open. Both branches are local only.
...
1. Finish the two substantive T3 security-ledger gaps: atomic pre-actuation reservation for ordinary generation calls, and concurrent vector upsert/prune plus dimension/HNSW integrity. Preserve held heads `367145b2` and `a272c8dc`, and include their fixes in the later current-upstream synthesis rather than pushing these source branches.
...
The prior documentation state was committed and pushed through `506ee953` on `feat/roundup-brainstorm`; this receipt supersedes its earlier T3 head and test counts. The web residual branch `fix/plan-audit-web-residuals` at `4a4dc5bf`, API residual branch `fix/recent-plan-api-residuals` at `0a54857a`, API demo-refresh branch `feat/demo-refresh-scaffold` at `e894f20f`, API demo-isolation branch `feat/demo-project-isolation` at `b5b13d8f`, web demo-UI branch `feat/demo-project-ui` at `d7e490ec`, API D5 branch `fix/pr-party-credential-rewrap` at `f3c4251d`, API T3 branch `fix/t3-security-api` at `367145b2`, web T3 branch `fix/t3-security-web` at `a272c8dc`, API T1 synthesis branch `upstream-queue/t1-api-synthesis` at `d2c31aea`, and web T1 synthesis branch `upstream-queue/t1-web-synthesis` at `4986135c` are durable local branches only; none is pushed or merged.
