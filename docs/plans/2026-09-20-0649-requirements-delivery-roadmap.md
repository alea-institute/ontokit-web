...
**D02: recovery verified; hosted identity access pending. Independent deliverable D06: B10 full-stack test foundation (locally verified; final review and publication next).**
...
- D06's first real probes found a default API Dockerfile defect: the package directory is root-owned `0644`, preventing non-root imports and migrations. [Lifecycle evidence](../releases/d06-lifecycle-readiness.md) records the reproduced failure and successful cleanup. The repair in `.worktrees/api-d06-image` is locally committed as `4d17d949`; its full-image non-root smoke and fresh migrations pass. Review/publication remain pending before activating the DEV candidate. U1 normal/setup-failure/late-failure/INT/TERM/crash-recovery probes all pass, with no owned leftovers and unchanged neighboring resource identities.
...
| D06 | B10 isolated local full-stack foundation | Implementing | [Reviewed plan](2026-09-20-1357-feat-isolated-full-stack-tests-plan.md); checkpoint `db916292` | U1–U5 lifecycle, authentication, API and browser workflows verified; U6 repeatability verified; final review/publication pending; real local services and disposable identity, API/browser workflows and repeatable cleanup; B11–13 broader coverage remains separate |
...
| B10 | Establish repeatable full-stack API and browser tests with isolated projects, real backend state, setup and cleanup | Implementing | D06 reviewed plan; lifecycle isolation first, then real authentication and all API/browser categories; acceptance pending |
