...
describe("TrustLadderSection", () => {
  ...
  it("saves only after an edit, and PATCHes only the touched settings", async () => {
  ...
  it("preserves edits made while a save is pending", async () => {
    const user = userEvent.setup();
    let resolveUpdate: (value: ProjectTrustSettings) => void = () => {};
    mockedUpdate.mockImplementation(
      () =>
        new Promise<ProjectTrustSettings>((resolve) => {
    ...
    await act(async () => {
      resolveUpdate(settings({ auto_accept_enabled: true }));
    });

    await waitFor(() => expect(threshold.value).toBe("9"));
    expect(
      (screen.getByRole("button", { name: /Save trust settings/ }) as HTMLButtonElement)
        .disabled,
    ).toBe(false);
  ...
  it("can retry after a load failure", async () => {
    const user = userEvent.setup();
    mockedGet.mockRejectedValueOnce(new Error("nope")).mockResolvedValueOnce(settings());
    renderWithQueryClient(
      <TrustLadderSection projectId="p1" accessToken="tok" canManage />,
    );
    ...
    expect(await screen.findByLabelText(/Accepted suggestions to become trusted/)).toBeDefined();
