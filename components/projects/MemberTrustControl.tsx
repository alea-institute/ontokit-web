  ...
  const apply = useCallback(
    async (next: TrustOverride) => {
      ...
      setOptimistic({
        ...current,
        trust_override: next,
        is_trusted: next === "granted" ? true : next === "none" ? current.is_trusted : false,
        tier:
          next === "granted"
            ? "trusted"
            : next === "none"
              ? current.tier
              : current.tier === "reviewer"
                ? "reviewer"
                : "untrusted",
      });
    ...
    [current, optimistic, projectId, userId, token, onChanged, close],
  );
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div
          ...
          className="absolute right-0 z-20 mt-1 w-64 rounded-md border border-slate-200 bg-white py-1 shadow-lg dark:border-slate-700 dark:bg-slate-800"
        >
