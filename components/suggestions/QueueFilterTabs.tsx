...
import type { SuggestionQueue } from "@/lib/api/suggestions";
import { cn } from "@/lib/utils";

/** "all" is the pre-ladder behaviour — the server sees no `queue` param. */
export type QueueFilter = SuggestionQueue | "all";
...
export function QueueFilterTabs({ value, onChange, counts, disabled }: QueueFilterTabsProps) {
  return (
    <div
      ...
      className="flex max-w-full overflow-x-auto rounded-md border border-slate-200 bg-white p-0.5 sm:inline-flex dark:border-slate-700 dark:bg-slate-800"
    >
      {OPTIONS.map((option) => {
        ...
        return (
          <button
            ...
            title={option.hint}
            className={cn(
              "flex shrink-0 items-center gap-1.5 rounded-sm px-3 py-1.5 text-sm font-medium transition-colors",
